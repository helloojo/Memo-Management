window.onload = function() {
	var MemoManage = (function() {
		...
		function makelist(board, memo) { // board list출력 함수
			...
			if ($boardlist.accordion()) { // 새로운 DOM 요소가 추가되었으므로 accordion 이벤트
											// 다시설정
		...
		function setBoardListEvent(btn) {
			...
			add.click(function() {
				DBConn.addMemotoDB($(this).parent().parent().data("boardid")); // memo
																				// 추가
			});
			edit
					.click(function() { // board 수정
			...
			dboard
					.click(function() { // 사용자에게 확인 후 board 삭제
